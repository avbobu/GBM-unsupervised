...
class Laplacian_regularization:
    def analysis(self, G, idxs, labeled_set, portion):
        n = G.number_of_nodes()
        laplacian_matrix = nx.normalized_laplacian_matrix(G)
        vals, vecs = sparse.linalg.eigs(laplacian_matrix.asfptype(), k=int(portion * n), which = 'SM')
        
        vecs = [vecs[:, i] for i in idxs]
        print(len(vecs[0]))
        vals = vals[idxs]
        n_idxs = len(idxs)
        ...
        def q(a):
            ...
            for i in labeled_set:
                s += (G.nodes[i]['label'] - sum(a[j] * vecs[j][i] for j in range(n_idxs)))**2

            return s 

        a0 = np.zeros(n_idxs)
        # min_res = optimize.basinhopping(q, a0, disp = True)
        min_res = optimize.minimize(q, a0, options = {'disp': True})
        a_opt = min_res.x
        print(a_opt)

        optimal_vector = np.dot(a_opt, vecs)
    ...
    def __init__(self, G, vec_idxs = [], labeled_set = [], eta = 0.02):
        self.accuracy = 0
        n = G.number_of_nodes()
        ...
        if len(labeled_set) == 0:
            labeled_set = random.sample(list(G.nodes), int(eta * n))
        for u in labeled_set:
            if G.nodes[u]['ground_label'] == 1:
                G.nodes[u]['label'] = 1
            else: 
                G.nodes[u]['label'] = -1

        # The algorithm 
        self.analysis(G, idxs = vec_idxs, labeled_set = labeled_set, portion = 0.02)
    ...
    def __init__(self, G, n = 2000, portion = 0.02, vectors = [13], spectrum_disp = False, cut_disp = False, vectors_disp = False, k_means_disp = False, sigma = 0.5):
        ...
        if cut_disp:
            for i in vec_idxs:
                ...
                mean_dists = {0: 0}
                for v in G:
                    dists_v = [dist_dict[u] for u in G.neighbors(v)]
                    mean_dists.update({v: sum(dists_v)/len(dists_v)})
                dist = [mean_dists[x] for x in sorted(dist_dict)]
                plt.scatter(dist[1:int(n/2)], vector[1:int(n/2)], color = 'blue')
                plt.scatter(dist[int(n/2):], vector[int(n/2):], color = 'blue')
                plt.title("i = " + str(i) + ", eigenvalue = " + str(vals[i]) + ", accuracy = " + str(accuracy))
                plt.show()              
